...
class Brickhouse extends BHEmitter {
  ...
  constructor(server, opts) {
     ...
     * @type {Object}
     */
    this._opts = _.extend({}, require('./../config'), opts);
    ...
    debug('Instantiated Brickhouse plugin with options %j and boards %j',
      this._opts, this.boards);
  }
  ...
  createBoard(config, id) {
    ...
    debug('Brickhouse#createBoard called with ID "%s" and config %j', id,
      config);

    id = config.id = (_.isString(id) && id) || config.id || null;

    board = new Board(server, config)
