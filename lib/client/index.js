...
class DigsClient extends DigsEmitter {
  ...
  start() {
    ...
    return (this._ready = new Promise(function (resolve, reject) {
      try {
        ascoltatori.build(opts, resolve);
      } catch (err) {
        reject(err);
      }
    })
  ...
  subscribe(path, opts) {
    return this._ascoltatore.subscribeAsync(String(this._topic(path, opts)));
  }
  ...
  request(path, message) {
    ...
    debug(`${this}: created request "${reqId}" on topic "${topic}"`);

    // add it to the queue
    this._enqueueRequest(topic, message);

    return new Promise(function (resolve) {
      debug(`${this}: waiting for response for request "${reqId}" on topic
      "${topic}"`);

      this.on(topic.event, function (message) {
        debug(`${this}: response "${reqId}" on topic "${topic}" received`);
