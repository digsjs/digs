...
class Peon extends events.EventEmitter {
  constructor() {
     ...
     */
    this.components = {};
    this.id = process.env.BRICKHOUSE_ID;
  ...
  connect() {
    let host = process.env.MQTT_HOST,
      port = _.parseInt(process.env.MQTT_PORT);
    this.client = new MQTTClient(this.id, host, port, 'slave');

    _.each(['init', 'instantiate', 'dir', 'execute'], function (funcName) {
      this.client.on(funcName, function (message, reply) {
  ...
  init(opts) {
    ...
    let board = this._board =
      new j5.Board(opts);
    pipeEvent(['error'], this._board, this);
