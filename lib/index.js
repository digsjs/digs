...
function digs(config, options, done) {
  ...
  return fs.readFileAsync(configPath, 'utf8')
    ...
    .spread((server, restore) => {
      ...
      server.log('debug', 'Configuring Seneca...');
      const home = require('user-home');
      const storepath = join(home, `.${server.settings.app.namespace}`,
        server.settings.app.project, 'data');
      server.log('debug', `Creating dir ${storepath} if it doesn't exist...`);
      return fs.mkdirp(storepath)
        .return([server, storepath]);
    })
    .spread((server, storepath) => {
      server.seneca.use('jsonfile-store', {
        folder: storepath
      });
      server.log('debug', 'Seneca configured.');
